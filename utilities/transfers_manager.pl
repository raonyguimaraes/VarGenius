...
#my $folder = $ARGV[0];#folder to search

#my $folder  ="/home/ngsworkspace/runs/MotorPlex_v7_18122015/Unaligned/Project_Motor/";

#Writes the sample sheet for a given folder searching for folders inside
#and searching for lanes _L001_ to _L004_
#write_sample_sheet_lanes($ARGV[0],$ARGV[1],$ARGV[2]);#NEEDS THE HEADER FILE
#INPUT	header - header file
	#     out_dir - directory of the samples
	#     more - motor or exome
#write_sample_sheet_both($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3]);#NEEDS THE HEADER FILE
	#     mult_lanes - if there are multiple lanes or not (use "lanes" if yes)
	#	filenames must be in the following form: AAA_L00X_BBB.fq

#write_sample_sheet_both_selected($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4]);#NEEDS THE HEADER FILE
	
#Writes the sample sheet for a given folder searching for folders inside
#write_sample_sheet($ARGV[0],$ARGV[1],$ARGV[2]);#NEEDS THE HEADER FILE
#INPUT	$header - header file
	#     out_dir - directory of the samples
	#     more - motor or exome
	
#merge_fastq($ARGV[0],$ARGV[1]);

#This tool can download from a basespace directory files that are separated
#by lane and directly merges them in the destination folder
#find_and_merge_from_basespace_dl($ARGV[0],$ARGV[1],$ARGV[2]);
#INPUT	$dir - directory source
	#     out_dir - destination directory
	#     fileList - file with folder names to take
	
#download_selected_motor($ARGV[0],$ARGV[1],$ARGV[2]);

#Writes the sample sheet for a given folder searching for folders inside
#using the format of the old pipeline
#INPUT	$header - header file
	#     out_dir - directory of the samples
	#     more - motor or exome
#write_old_sample_sheet($ARGV[0],$ARGV[1],$ARGV[2]);

#copy_selected($ARGV[0],$ARGV[1],$ARGV[2]);
#move_selected($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3]);
...
#=head2 download_selected_motor

 #Title  : download_selected_motor
 #Usage  : download_selected_motor( 
									#- dirname => 'the folder where to copy',
                  #- fileList => list of samples to download
									#- destDir => destination folder
                      #);
...
#sub download_selected_motor {
	...
	#my @list = list_to_array($fileList,'NO_NEW_LINE');

	##Opens the dir with all the samples
	#opendir DIR, $dir or die "ERROR : cannot open dir $dir";
	#my @samples_dirs= readdir DIR;#All the files are in this list
	#print "Downloading from: $dir\n";
	#foreach my $sample ( @samples_dirs){

			#$sample =~ s/Motor\_//;
			#print "Searching for $sample..\n";
			#if ( grep {/$sample/} @list){
					#if (! (-d $destDir."/Motor\_".$sample) ){
						#my $command = "cp -Lr $dir/Motor\_".$sample." $destDir";
						#print "Executing: $command\n";
						#try_exec_command($command);
					#}else{
						#print "Unable to copy ".$destDir."/Motor\_".$sample.". Folder exists...\n";
						#}
			#}
	#}
#}
...
sub download_selected {
	...
	print_and_log ( "Finished copying $total_samples samples with $diff_md5 errors in $partDuration seconds. \n",$log_file);
}
