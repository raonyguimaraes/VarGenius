...
#Works at CINECA
use lib '/pico/work/TELET_UDP/VarGenius'; #PICO CINECA
	...
	while ( scalar(keys %{$deps}) > 0 ){
		...
		while (my $line = <QSTAT_OUT>){
				if( $start == 1 ){
					...
					#print "4: ".$fields[3]." - 5: ".$fields[4]." - 6:".$fields[5]." " ;#DEBUGCODE
					#print "8: ".$fields[7]." - 9: ".$fields[8]." - 10:".$fields[9]."\n" ;#DEBUGCODE
				}
				if ($line =~ /---------------/){
					$start = 1;
				}
		...
		foreach my $job_name ( keys %{$deps}){
			...
			#If they have finished (or gone out) the subject job will be killed
				...
				if ( $exited == $num_dep_jobs){
						...
						my ($task,$type,$name) = get_info_from_jobname($cfg_hash,$job_name);
...
if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
	...
	foreach my $analysis_id ( @analysis_ids ){
		...
		if( grep {/\b$task\b/} @tasks_to_exec ){
			...
			#If the user ran also the BaseRecalibration then the bam files produced by IndelRealigner 
			#are now useless
			##Verifying that there has been an execution of multiple samples
			#my $PR_step = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
									#$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'print_reads_step'}
									#,$cfg_hash->{'db_sample_id'},$sample_id);
			#if ($PR_step == 1){
				#Remove the bam files output of IndelRealigner
			...
			#}else{
					#print_and_log( $cfg_hash->{'base_recalibrator_prog'}." has not been ran.".
					#$cfg_hash->{'indel_real_step'}.".".$cfg_hash->{'bam_ext'}." files and ".
					#$cfg_hash->{'base_recal_step'}.".".$cfg_hash->{'recal_ext'}." files ".
