...
#Files manipulation
use LIB::files_manipulation qw(  append_file_2_file);
...
use LIB::programs_management qw(hash2ConfigFile configFile2Hash try_exec_command 
				...
				show_samplesids_from_samplesnames);
...
use LIB::db_management qw(createDBAndTables getSampleConfiguration copyTable2DB
				...
				select_all_from_db db_present select_distinct_samples createTables
				 do_query_select_all update_table
...
if ( -e $user_config and $user_config ne ''){
 ...
 if ( $license ){
	...
	if ( $exec_utils > 0){
			...
			if ( $save_analyses ne '' ){
				store_analyses($cfg_hash,$working_folder,$store_analyses,$force_run,$log_file);
			}					
...
sub pipeline_execution{
	...
	if ( $do_pipeline_tasks ){
		...
		foreach my $analysis_id ( keys %{$exec_hash}){
			...
			#print_and_log("$analysis_indication2 Changing the configuration hash \n",$log_file);#DEBUGCODE
			my $cfg_hash_gr;
			...
			####PEDIGREE FILE
			#print_and_log("Creating the Pedigree file..\n",$log_file);#DEBUGCODE
			##If we could create the pedfile, then also the phasing can be done
			##unless the user decides to avoid it
			#if( $cfg_hash_gr->{'phasing'} eq 'YES'){
				...
				####Create pedfile and put it in the folder###
				##Set the path of the file   
				#my $out_ped = $cfg_hash_gr->{$analysis_id."_data_fold"}."/$analysis_name.".$cfg_hash->{'ped_ext'};		
				#if ( get_ped_file($cfg_hash_gr,$analysis_id,$out_ped,$log_file_gr) ){
					#print_and_log("$analysis_indication2 Created a pedigree file for the analysis \n",$log_file_gr);
					#$phasing = 1;
				#}
			#}
		 ...
		 ###PEDIGREE FILE
			...
			if ( $num_samples > $cfg_hash_gr->{'min_samples_4_joint'}){
				print_and_log("$analysis_indication2 Obtaining a PED file for $num_samples samples : $out_ped \n",$log_file);
			...
			#I create here the pedfile, if there is a trio (M,F,P) then also the phasing can be done
				...
				if( $cfg_hash_gr->{'phasing'} eq 'YES' and $trios == 2){
					print_and_log("$analysis_indication2 The analysis is of a TRIO. Phasing will be performed \n",$log_file_gr);
					$phasing = 1;
				}else{
					print_and_log("$analysis_indication2 The analysis is not of a TRIO. Phasing will not be performed \n",$log_file_gr);
					$phasing = 0;	
				}									
			}
			...
			#Check if it exists								
			if (file_not_present($target_bed_path) > 0 ){ log_and_exit("$analysis_indication2 ERROR: $target_bed_path does not exist. Please put the target file into ".$cfg_hash_gr->{'target_reg_f'}." ...\n",$log_file_gr);}		
			...
			my $target_id = get_id_if_exists_from_db($cfg_hash_gr->{'db_name'},$cfg_hash_gr->{'db_dsn'},								
											$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_target_files_table'},$cfg_hash->{'db_targetid'},
			...
			my $perchrom = get_id_if_exists_from_db($cfg_hash_gr->{'db_name'},$cfg_hash_gr->{'db_dsn'},$cfg_hash_gr->{'db_user'},
				...
				if ( $db_target_perchrdiv == 0 ){
					...
					unless(-d $dest_folder){
						print_and_log("$analysis_indication2 Creating folder $dest_folder...\n",$log_file_gr);
						mkdir $dest_folder or die "$analysis_indication2 ERROR: can't create folder $dest_folder. Check permissions. \n";
					}	
				...
				$cfg_hash_gr->{'chrom_reg_f'}	= $dest_folder;
				print_and_log("$analysis_indication2 Chromosomal regions will be pickedfrom: ".$cfg_hash_gr->{'chrom_reg_f'}."..\n",$log_file_gr);
			#} 			    
			...
			if (file_not_present($target_int_list) > 0 ){
				...
				print_and_log("$analysis_indication2 Executing $prog_used to generate the ".$cfg_hash_gr->{'target_inter_list_ext'}." file for $target_bed : ",$log_file_gr);
			...
			#Store the config hash in the data folder for the group
			my $cfg_hash_f = $cfg_hash->{$analysis_id."_data_fold"}."/cfg_$analysis_id.hash";
			print_and_log("$analysis_indication2 Storing the configuration hash in $cfg_hash_f..\n",$log_file_gr);
			save_hash(\$cfg_hash_gr,$cfg_hash_f);
			$cfg_hash->{'cfg_hash_f'} = $cfg_hash_f;



      ################################
      #PIPELINE EXECUTIONG FOR EACH SAMPLE
      ###################################
      ...
      #For each sample in each analysis
			while (my ($sample_id, $value) = each %{ $exec_hash->{$analysis_id} } ) {
				...
				if ($qualityCheck){
					...
					print_and_log($niceClosure,$log_file_gr);
					my $analysis_indication = "[an:$analysis_id sam: $sample_id ](".scalar(localtime)."): ";
					#print_and_log("$analysis_indication: $task step \n",$log_file_gr);#DEBUGCODE

					#Get dependencies
					my $dependency = "no_depend";
					#Loop through sample ids so that it will run a job for each sample
					foreach my $readf_id (@readf_ids){
						$analysis_indication = "[an:$analysis_id sam: $sample_id rf: $readf_id](".scalar(localtime)."): ";
						print_and_log("$analysis_indication Launching a job for $task for read file $readf_id : ",$log_file_gr);
						...
						my $job_id = try_exec_job( $cfg_hash_gr,$env_vars,$task,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file_gr);
						#print_and_log("Qsub id: $job_id\n",$log_file_gr);
						print_and_log("$analysis_indication - step:$task - dependencies:$dependency - jobid:$job_id]\n",$log_file_gr);
						#Put this job id in the hash for dependencies
						$dependence_hash->{$job_name} = $job_id;		
				...
				if ($alignment){
					...
					foreach my $readf_id (@readf_ids){
						...
						print_and_log("$analysis_indication Launching a job for $task : ",$log_file_gr);
				...
				if ($refining){
					...
					if ($cfg_hash_gr->{'realign_target_creator'} eq 'YES'  or $cfg_hash_gr->{'indel_realigner'} eq 'YES'){
						...
						foreach my $readf_id (@readf_ids){
							...
							my $env_vars = "CONFIG_FILE=$cfg_hash_f,TASK=$task,SAMPLE_ID=$readf_id,ANALYSIS_ID=$analysis_id,LOG_FILE=$log_file_gr,PIPE=$pipeline ";
							my $job_id = try_exec_job( $cfg_hash_gr,$env_vars,$main_task,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file_gr);						
							#print_and_log("Qsub id: $job_id\n",$log_file_gr);
							print_and_log("$analysis_indication - step:$task - dependencies:$dependency - jobid:$job_id\n",$log_file_gr);
				...
				#Merge read files in unique files for each sample
				...
				#run this step when the variable multiple is 1. In this way the user does not have to remember
				...
				if ( $varcall and 
					...
					print_and_log($niceClosure,$log_file_gr);
					#print_and_log("$analysis_indication  $task step \n",$log_file_gr);#DEBUGCODE
					print_and_log("$analysis_indication Checking if is useful to apply mergesam to your samples..\n",$log_file_gr);
					...
					#Verify if there are multiple read files
					if ( $multiple == 1){
						$mergesam = 1;#To activate the wait in variant calling step
						print_and_log("$analysis_indication Sample $sample_id is subdivided in multiple read files. Merging results..\n",$log_file_gr);
		
						#Managing the dependencies
						my $task_to_wait;
						#If no refine step must be executed before, then the task to wait is the alignment
						if (( $cfg_hash_gr->{'realign_target_creator'} eq 'NO' and $cfg_hash_gr->{'indel_realigner'} eq 'NO'
								and $cfg_hash_gr->{'base_recalibrator'} eq 'NO' and $cfg_hash_gr->{'print_reads'} eq 'NO'
				...
				if ( $varcall ){
					...
					###For each group we check if there are multiple sample. It it is, a Joint Genotyping will be run###
					...
					if ( $mergesam ) {
						...
						#Otherwise wait for the indel realignment
						else{
							$task_to_wait = "IR";
						}
					...
					#print_and_log("$analysis_indication This job will wait for these jobs: $dependency\n",$log_file_gr);#DEBUGCODE
					print_and_log("$analysis_indication Launching a job for $task : \n",$log_file_gr);
      ...
      ###################################
			...
			my $consensus = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
											...
											$cfg_hash->{'db_analysis_id'},$analysis_id);
			if ( $varcall and ($cfg_hash->{'consensus_vc_approach'} eq 'YES' or $consensus == 1) ){
				...
				if ($cfg_hash->{'vc_with_samtools'} eq 'YES' ){
					...
					my $job_log = $cfg_hash_gr->{$analysis_id.'_'.$task_4_fold.'_log_fold'}."/".$analysis_name."_samtools-mpileup_pipe.log";
				...
				if ($cfg_hash->{'vc_with_freebayes'} eq 'YES' ){
					...
					my $job_name = $task."_a".$analysis_id;
					my $job_log = $cfg_hash_gr->{$analysis_id.'_'.$task_4_fold.'_log_fold'}."/".$analysis_name."_freebayes_pipe.log";
					my $env_vars = "CONFIG_FILE=$cfg_hash_f,TASK=$task,SAMPLE_ID=$analysis_name,ANALYSIS_ID=$analysis_id,LOG_FILE=$log_file_gr,PIPE=$pipeline ";
					my $job_id = try_exec_job( $cfg_hash_gr,$env_vars,$task,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file_gr);						
					#print_and_log("Qsub id: $job_id\n",$log_file_gr);
					print_and_log("$analysis_indication step:$task - dependencies:$dependency - jobid:$job_id\n",$log_file_gr);
			...
			if ($fastfout){
				...
				print_and_log($niceClosure,$log_file_gr);
				#print_and_log("$analysis_indication  $task step\n",$log_file_gr);#DEBUGCODE	
				#print_and_log("$analysis_indication Generating a VCF and annotating it ..\n",$log_file_gr);

				#Managing the dependencies
				my $task_to_wait;
				
				#CHECKING THE PHASING PERFORMED
				##Getting the pedigree file to check if this is a trio
				#my $ped_file = get_id_if_exists_from_db($cfg_hash_gr->{'db_name'},$cfg_hash_gr->{'db_dsn'},$cfg_hash_gr->{'db_user'},
									#$cfg_hash_gr->{'db_pass'},$cfg_hash_gr->{'db_analyses_table'},$cfg_hash_gr->{'db_pedfile'},
									#$cfg_hash_gr->{'db_analysis_id'},$analysis_id);	
				##If exists the pedigree file, then the last step was the phasing
				#my $ped_f_path = $cfg_hash_gr->{$analysis_id.'_data_fold'}."/$ped_file";
				##If the analysis is of a Trio then the phasing may be executed
				#if ( -e $ped_f_path ){
				...
				my $job_name = $task."_a".$analysis_id;
				my $job_log = $cfg_hash_gr->{$analysis_id.'_'.$maintask.'_log_fold'}."/".$analysis_name."_".$cfg_hash_gr->{$maintask.'_log'};
				my $env_vars = "CONFIG_FILE=$cfg_hash_f,TASK=$task,SAMPLE_ID=$analysis_name,ANALYSIS_ID=$analysis_id,LOG_FILE=$log_file_gr,PIPE=$pipeline ";
				my $job_id = try_exec_job( $cfg_hash_gr,$env_vars,$task,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file_gr);						
				#print_and_log("Qsub id: $job_id\n",$log_file_gr);
				print_and_log("$analysis_indication step:$task - dependencies:$dependency - jobid:$job_id\n",$log_file_gr);
				...
				if ($job_id ne ''){
					...
					my $ss_id = get_id_if_exists_from_db($cfg_hash_gr->{'db_name'},$cfg_hash_gr->{'db_dsn'},$cfg_hash_gr->{'db_user'},
							...
							$cfg_hash_gr->{'db_analysis_id'},$analysis_id);
					print_and_log("$analysis_indication Setting analysis $analysis_id (jobid:$job_id) to be the last waiter of the db for the final output ...\n",$log_file_gr);
					...
					log_and_exit("$analysis_indication ERROR: Cannot get a correct jobid. Check if there are problems with job execution.\n",$log_file_gr);
				}
			...
			if ($vcfimport){
				...
				print_and_log($niceClosure,$log_file_gr);
				print_and_log("$analysis_indication  $task step\n",$log_file_gr);	#DEBUGCODE
				#print_and_log("$analysis_indication Importing VCF in DB for analysis $analysis_id..\n",$log_file_gr);#DEBUGCODE
				...
				if ($cfg_hash->{'vcfimport'} eq 'YES'){
					...
					if ( scalar(keys %{$anal_ids_fetch}) > 0 ){
						print_and_log("$analysis_indication WARNING: Variants from analysis  $analysis_id will not be imported. I found ".scalar(keys %{$anal_ids_fetch}).
						" genotypes into the ".$cfg_hash->{'db_genotype_sample_table'}." table. Please check this information. Exiting...\n",$log_file_gr);
					}					
				}
				...
				if ($job_id ne ''){
					...
					#The VCF import can be always blocked with this parameter.
 ...
 # "-m|--mergesam: \n".
  ...
  if ( $VERSION ){
	...
	if (scalar(@tasks_to_exec) ){
		#Check if taskVCFBs to execute are all consecutives
		chop($formed_str);
		if ( $all_tasks_str !~ $formed_str and $doanalysis == 1){
