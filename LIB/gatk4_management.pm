...
=head2 run_GATK4_HaplotypeCaller

 Title   : run_GATK4_HaplotypeCaller
 Usage   : run_GATK4_HaplotypeCaller(   );

 Function: run_GATK4_HaplotypeCaller
...
sub run_GATK4_CalculateGenotypePosteriors{
	...
	my $command = $jatk_call." --variant $input_file -R ".$cfg_hash->{'hum_ref'}." $param_str -ped $ped_file -o $outFile ";			 
	print_and_log("Executing command: $command\n",$log_file);
	try_exec_command($command) or die "Unable to execute command: $command\n";	
}
...
####### JAVA PARAMETERS	
	...
	my $command = $jatk_call." -R ".$cfg_hash->{'hum_ref'}." -input $input_file $res_string $ann_string -mode $mode -recalFile $recal_file -tranchesFile $tranches_file ".
...
=head2 run_GATK4_ApplyRecalibration

 Title   : run_GATK4_ApplyRecalibration
 Usage   : run_GATK4_ApplyRecalibration(   );

 Function: run_GATK4_ApplyRecalibration
			Is a tool from GATK which permits to recalibrate variants. It can work
			per-sample 
			
 Returns : nothing
=cut
sub run_GATK4_ApplyRecalibration{
	my $cfg_hash = shift;
	my $prog_used = shift;
	my $sample_id = shift;
	my $group_id = shift;
	...
	#print_and_log( "$jatk_call - gatk params: $picard_params\n",$log_file);
	my $command = $jatk_call." --input $input_file -R ".$cfg_hash->{'hum_ref'}." -mode $mode -recalFile $recal_file -tranchesFile $tranches_file ".
								"$param_str  -o $out_vcf ";			 
	print_and_log("Executing command: $command\n",$log_file);
	try_exec_command($command) or die "Unable to execute command: $command\n";
}
