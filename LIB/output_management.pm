...
use LIB::files_management qw(save_hash load_hash file_not_present 
														...
														extract_name twocolfile_to_hash);
#
use LIB::files_manipulation qw( list_to_array append_str_2_file_if_path_notexist );
		...
		for (my $alt_all = 0; $alt_all < scalar(@alts); $alt_all++){
			...
			foreach my $sample (@sort_samples){
				...
				foreach my $format_field (@format_fields_needed){
					...
					#if ( grep {/\b$lc_f_f\b/} @format_fields_needed){
...
sub output_from_annotation_without_db {
  ...
  my $samples_h;#Samples hash to be reordered
	my @sort_samples = ();#The array with samples id reordered
	#Get the samples ids involved for the group
	my $query = "SELECT ".$cfg_hash->{'db_sample_id'}." FROM  ".$cfg_hash->{'db_sample_table'}." WHERE ".$cfg_hash->{'db_analysis_id'}."=$group_id;";	
	#print_and_log( "Executing: $query\n",$log_file);#DEBUGCODE
  ...
  if ( ($genotref == 1) and ($cfg_hash->{'run_samples'} ne 'ALL') ){
	...
	foreach my $sample_name (@sort_samples){
		#print_and_log("Getting genotype header for the sample id: $sample_id..\n" ,$log_file);#DEBUGCODE
	...
	while (my $line = <ANN_OUT>){ # and ($num_line < 1000){#DEBUGCODE
		...
		###########################
			...
			if ( scalar(@ann_fields) ne ($tot_cols+$tot_info_annovar)){
				print_and_log("WARNING: Line $num_line contains ".scalar(@ann_fields)." fields while they should be ".$tot_cols+$tot_info_annovar."..\n",$log_file);				
			}
...
sub separate_analysis_from_joint {
	...
	foreach my $sel_sample_id (@samples_ids){
			my $sample_name = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
							$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'db_sample_name'},
							$cfg_hash->{'db_analysis_id'}.",".$cfg_hash->{'db_sample_id'},"$group_id,$sel_sample_id"); 
...
=head2 import_vgout_2_db

 Title   : import_vgout_2_db
 Usage   : import_vgout_2_db( -database => 'name of the database,
                               );

 Function: Inserts in the database selected results from the rearranged output of VarGenius
					BE CAREFUL: The list of fields to import (fields_to_import) in program_config.txt 
					must contain the same order of fields as they are into the vargenius rearranged output
					Because they are used in the query as well
					
 Returns : 

=cut
sub import_vgout_2_db {
	my $cfg_hash = shift;
	my $ann_out = shift;
	my $prog_used = shift;
	my $analysis_id = shift;
	...
	my $num_line = 0;
	my $tot_cols = 0;
	#The fields that will be inserted in the db
	my $fields_2_imp = $cfg_hash->{'db_var_id'}.$comma.$cfg_hash->{'fields_to_import'};
	#Get the needed fields
	my @needed_fields = split(",",$cfg_hash->{'fields_to_import'});
	my @needed_fields_inds = ();#indexes found for func columns
	...
	print_and_log("Analysis of variants (line): ",$log_file);
	#Go through each line of a TXT tab separated table
	while (my $line = <ANN_OUT>){
		...
		if ($num_line == 0){
			...
			foreach my $head_fld (@fields){
				...
				if ( grep {/\b$head_fld\b/} @needed_fields ) {
					#Add the index where to find the func cols
					push(@needed_fields_inds,$num_col);	
				}							
				$num_col++;
			}
		}##########HEADER###########		
	...
	###########DATA############
			...
			my $compid = $fields[$cfg_hash->{'vcf_chr_ind'}]."_".$fields[$cfg_hash->{'vcf_pos_ind'}]."_".$fields[$cfg_hash->{'vcf_ref_ind'}]."_".$fields[$cfg_hash->{'vcf_alt_ind'}];
			...
			for ( my $col=0; $col< scalar(@fields); $col++){		
				...
				if ( grep {/\b$col\b/} @needed_fields_inds ) {
					#Add this field to the values 
					$values .= "'".$fields[$col]."'$comma";
				}
			}
			chop($values);
			#Check if the varid is a positive number
			if ( correct_type($varid,"positiveint") ){
				#print_and_log("Importing FIELDS: $fields_2_imp and VALUES: $values\n",$log_file);
				##Put the information in the table if the id is not present
				##If it is already there, it takes its identifier
				my ($new_ann_id,$exist_ann_id) = insert_only_if_not_exists_locked_woconn($dbh,$cfg_hash->{'db_var_annotations_table'},$cfg_hash->{'db_ann_id'},
									$fields_2_imp,$values,$cfg_hash->{'db_var_id'},$varid); 	
				#I will insert only if it is not present in the DB				
			  if ( $new_ann_id < 0){
					#print_and_log(" [found]\n",$log_file);
					$found++;
				}else{
					#print_and_log(" [inserted]\n",$log_file);	
					$imported++;
				}				
			}
	...
	#Disconnect db
  $dbh->disconnect();
  print_and_log(" Finished variant annotation import\n",$log_file);
  print_and_log(" Newly imported: $imported - Already present: $found \n",$log_file);
}
...
=cut
sub import_annovarout_2_db_fastOLD {
	my $cfg_hash = shift;
	my $ann_out = shift;
	my $prog_used = shift;
	my $analysis_id = shift;
	...
	###########DATA############
			...
			for ( my $col=0; $col< scalar(@fields); $col++){
				...
				if ( grep {/\b$col\b/} @func_cols_inds ) {
					...
					$values .= "$transcript$comma$exon$comma$nt_change$comma$aa_change";						
...
=head2 import_annovarout_2_db

 Title   : import_annovarout_2_db
 Usage   : import_annovarout_2_db( -database => 'name of the database,
                               );
	...
	###########DATA############
			...
			for ( my $col=0; $col< scalar(@fields); $col++){
				...
				if ( grep {/\b$col\b/} @func_cols_inds ) {
					...
					my @aachange_fields = split(":",(split($comma,$aachange_column)) [0]);
					if (scalar(@aachange_fields) > 0){
						my $transcript = $aachange_fields[1];
						my $exon = $aachange_fields[2];
						my $nt_change = $aachange_fields[3];
						my $aa_change = $aachange_fields[4];
						
						$values .= "'$transcript'$comma'$exon'$comma'$nt_change'$comma'$aa_change'";						
					}else{
						$values .= "'$empty_val'$comma'$empty_val'$comma'$empty_val'$comma'$empty_val'";	
					}
...
=head2 import_ann_out_2_db

 Title   : import_ann_out_2_db
 Usage   : import_ann_out_2_db( -database => 'name of the database,
                               );

 Function: Inserts in the database all the results from annotation with Annovar
						example of output from the annotation with Annovar
						Chr,Start,End,Ref,Alt,Func.refGene,Gene.refGene,GeneDetail.refGene
						chr1,13302,13302,C,T,"ncRNA_exonic","DDX11L1",".",.,.,"intronic"
					
					All the fields are converted:
					 upper to lowercase and all - and . into _
...
sub import_ann_out_2_db_OLD {
	...
	print_and_log( "Parsing Annotation output: $ann_out...\n",$log_file);#DEBUGCODE
	print_and_log( "Inserting annotation only for those variants which has not been annotated yet.\n",$log_file);#DEBUGCODE
	#Separators
	my $sep = "\t";
	my $db_sep = ",";
	my $tab = "\t";
	...
	#Open output from the annotation
	open (ANN_OUT,"<".$ann_out) or die "ERROR: Cannot open $ann_out. The program will exit..\n";
	#This field contains the transcripts information
	my $tr_ann_field_col = -1;
	my $tr_ann_field = $cfg_hash->{'tr_ann_field'};
	$tr_ann_field =~ s/[\-\.\+]/\_/g;
	$tr_ann_field = lc($tr_ann_field);
	...
	#The array of field names
	my @head_flds = ();
	#the indexes of the fields that are to be used for the db
	my @flds_touse = ();
	...
	while (my $line = <ANN_OUT>){
		#Remove last \n
		chop($line);
		#Inizialize the values for this line
		my $values = "";
		##########HEADER###########
		#If we are in the header
		if ($num_line == 0){
			#Split the header to get the fields names
			@head_flds = split($sep,$line);
			$tot_cols = scalar(@head_flds);
			#Number of column will be inserted in the flds_touse
			my $num_col = 0;
			#Keep in count if a field has been already inserted
			...
			foreach my $head_fld (@head_flds){
				...
				if ($head_fld eq $other_info_col) {
					...
					push(@flds_touse,$num_col);
						...
						#Convert upper to lowercase
						$head_fld = lc( $head_fld);
						#If the field starts with a number (e.g. 1000g), add _
						if ($head_fld =~ /^\d/){ $head_fld= "_".$head_fld;}					
						
						#check if this field was already inserted
						if ( ! grep {/\b$head_fld\b/} @insd_flds) {
							#Keep the index of the field that keeps the transcript information
							#and adds the fields names for the transcript infos
							if ( $head_fld eq $tr_ann_field ){
								$tr_ann_field_col = $num_col;
								$fields .= $cfg_hash->{'tr_ann_fields'}.$db_sep;
							}#Transcripts info field
							$fields .= $head_fld.$db_sep;
							push(@insd_flds,$head_fld);
							#Save the indexes to use
							push(@flds_touse,$num_col);
						}else{
							print_and_log("WARNING: File $ann_out contains a repeated field: $head_fld.".
								      " The column number $num_col will not be used..\n",$log_file);
						}
			...
			#The last element must be the varid field so there is
			#no need to remove last $sep
			#chop($fields);
		}##########HEADER###########
		...
		###########DATA############
			...
			my @ann_fields = split($sep,$line);
			if ( scalar(@ann_fields) ne ($tot_cols+$tot_info_annovar)){
							print_and_log("WARNING: Line $num_line contains ".scalar(@ann_fields)." fields while they should be ".$tot_cols+$tot_info_annovar."..\n",$log_file);				
			}
			#Get the index of the last field Otherinfo
			my $last_field = $flds_touse[-1];
			foreach my $i (@flds_touse){
				#The last field contains the INFO fields and the last of them
				#will be the variant identifier in the database
				if ($i == $last_field) {
					$var_id = $ann_fields[$i + $tot_info_annovar];
					#Remove the last quote if present
					#if ($var_id =~ /\"$/) {
					#	chop($var_id);
					#}
					$values .= $var_id;
					...
					################################
					#Takes the transcript information
					#################################
					if ( $i eq $tr_ann_field_col ){
						my $subj_field = $ann_fields[$i];
						if ( ($subj_field ne $annov_empty_val) and (lc($ann_fields[$i]) ne 'unknown') ){#and (not (grep {/\b$subj_field\b/} split($comma,$cfg_hash->{'rg_null_values'}))) ) {
							#Change quotes with nothing
							$ann_fields[$i] =~ s/[\"\']//g;
							#Separates the transcript info fields
							my @tr_fields = split($comma,$ann_fields[$i]);
							#Separates the fields of the first info 
							my @first_tr_info = split(":",shift @tr_fields);
							#after the shift the array has one element less
							# we put the values of the first element in different fields
							#if an element is not there will be substituted with $vargenius_empty_val
							#foreach my $el (@first_tr_info){
							for (my $fld = 0; $fld < $cfg_hash->{'rg_symbol_fields'}; $fld++){
								if ( $fld < scalar(@first_tr_info) ) {
									$values .= "'".$first_tr_info[$fld]."'".$db_sep;
								}else{
									$values .= "'$vargenius_empty_val'".$db_sep;
								}
							}
							#and put the remaining transcript info in a single value if it is there 
							my $rem_tr_info = $vargenius_empty_val; 
					...
					##########################
						...
						#the value is there
						if ( $ann_fields[$i] ne $annov_empty_val) {
							#Change double quotes with single
							#$ann_fields[$i] =~ s/"/'/g;#Removeme
							$ann_fields[$i] =~ tr/"'//d;
							#Substitue commas with our separator (]---[) because they will influence the queries
							$ann_fields[$i] =~ s/,/$mult_ann_sep/g;
							#If the field still does not contain quotes just give it
							#if ( $ann_fields[$i] !~ /^".*"$/ and $ann_fields[$i] !~ /^'.*'$/ ){#Removeme
							if ( $ann_fields[$i] !~ /^'.*'$/ ){
							 $ann_fields[$i] = "'".$ann_fields[$i]."'";
							}
							$values .= $ann_fields[$i].$db_sep;
						}else{
							#the value is not there, hence
							#substitute the field with vargenius_empty_val (-)
							$values .= "'".$vargenius_empty_val."'".$db_sep;
		 ...
		 #print_and_log("Line $num_line - Varid: $var_id\n $fields = $values\n",$log_file);#DEBUGCODE
		##### TO SEE ALL THE VALUES AND FIELDS WE ARE INSERTING ##DEBUG CODE
		#my @all_f = split(",",$fields);#DEBUG CODE
		#my @all_v = split(",",$values);#DEBUG CODE
		
		#for (my $i=0;$i<scalar(@all_f);$i++){#DEBUG CODE
			##if (not defined $all_v[$i]){#DEBUG CODE
				##print_and_log( "ERROR $i value is not defined at line $num_line (field:".$all_f[$i].")\n",$log_file);#DEBUG CODE
			##}#DEBUG CODE
			#print_and_log( $all_f[$i]." = ".$all_v[$i]."\n",$log_file);#DEBUG CODE
		#}#DEBUG CODE
		########
		#print_and_log( "\n\n",$log_file);#DEBUG CODE
		...
		#Check if the varid is numeric, the annotation could contain a row with
		#additional information
		if ( correct_type($var_id,"positiveint") ){
		  #Insert if the annotation id does not exist
		  my $ann_id = insert_only_if_not_exists_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
					$cfg_hash->{'db_pass'},$cfg_hash->{'db_var_annotations_table'},$cfg_hash->{'db_ann_id'},$fields,
					$values,$cfg_hash->{'db_var_id'},$var_id);
					#ann_id is the annotations id, if it is -1 the annotation is already present for the variant
		  if ( $ann_id < 0 ){
				print_and_log("WARNING: the Annotation for variant id $var_id is already present and will not be inserted..\n",$log_file);
		  }			
		}
		...
		foreach my $all_analysis_id (@analysis_ids){
			...
			if ( scalar(@sample_ids) > 0 ){
				...
				foreach my $sample_id (@sample_ids){
					...
					if ( $multiple == 1){					
						$flagstat_aln = $cfg_hash->{'work_fold'}."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/$sample_name\_"
						...
						$flagstat_aln = $cfg_hash->{'work_fold'}."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/".build_input_name_from_executed($params,$step_needed,
											$params->{$main_name},$steps_array)."_".$step_needed.".".$cfg_hash->{'flagstat_ext'};		
					}
					#print_and_log("flagstat input: $flagstat_aln\n",$log_file);			
					
					my $flagstat_aln_sort_mrdup = $cfg_hash->{'work_fold'}."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/$sample_name\_"
...
sub get_statistics_tables {
	...
	my $sam_cov_summ_f = $cfg_hash->{'work_fold'}."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/$analysis_name\_DOC.".$cfg_hash->{'DOC_sam_sum'};
	...
	if ( -e $sam_cov_summ_f) {
		...
		extract_columns_from_file($sam_cov_summ_f.".temp","0,1,2,6,7,8,9,10",$sam_cov_summ_totab);
	...
	##Sample Reads Alignment statistics
	#my $anal_aln_stats_f = $cfg_hash->{'work_fold'}."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/$analysis_name\_$plot_name.txt.$analysis_id.totab";
	#append_str_2_file_if_path_notexist($outFolder."/".$cfg_hash->{'statstables_file'},"Alignment Statistics\t".$anal_aln_stats_f);
...
sub get_gene_coverage_above {
		...
		if ( -e $gene_summary  and !(-z $gene_summary)){
			...
			#my $group_genecove_above = $working_folder."/".$analysis_name."/".$cfg_hash->{'stats_out_f'}."/".$analysis_name."_DOC_".extract_name($sel_genes_f,1)."_".$doc_outs[0]."_above$percentage_above.".$cfg_hash->{'txt_ext'};
...
sub generate_igvsession_xml {
		...
		my $bam_desc = $cfg_hash->{"outlist_sortedbam_desc"};
		...
		#Open XML file		
		open (XML,">$xml_igv_session_out") or die "ERROR[?]: Cannot open $xml_igv_session_out\n";
		
		#Print the header of the XML giving indication of the genome
		print XML '<Session genome="'.$build_ver.'" hasGeneTrack="true" hasSequenceTrack="true" version="8">'."\n";
		print XML "\t".'<Resources>'."\n";
		...
		print XML "\t\t".'<Resource path="'.$vcf_link.'"/>'."\n";
		#Print BAM paths				
		while (my ($sample_name, $value) = each %{ $paths_hash->{$covpage} } ) {
			...
			if ( defined $bam_path) {
				if ( -e $bam_path){
					...
					print XML "\t\t".'<Resource path="'.$bam_link.'"/>'."\n";				
				}else{
...
sub print_alignment_statistics_table{
  ...
  if (-e $flagstat_aln_sort_mrdup and (!-z $flagstat_aln_sort_mrdup)  ){
	...
	if ( $at_least_one > 0 ){		
		...
		$success = 1;
		#Store the information for the current analysis
		print_and_log( "Writing: $new_row\t$tot_reads_align\t$duplicate_reads\t$properly_paired",$log_file);#DEBUGCODE
		print SAMPLE_STATS "$new_row\t$tot_reads_align\t$duplicate_reads\t$properly_paired\n";
		...
		if ( $at_least_one > 1 ){	
			...
			my $id = -1;
			my $fields =  $cfg_hash->{'db_sample_id'}.",".$cfg_hash->{'db_sampstats_duplicates'}.",".$cfg_hash->{'db_analysis_id'};
			my $values = "$sampleid,$duplicate_reads,$analysis_id";
			my ($new_id,$exist_id) = insert_only_if_not_exists_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
									$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_statistics'},$cfg_hash->{'db_sample_id'},
									$fields,$values,$cfg_hash->{'db_sample_id'},$sampleid);
