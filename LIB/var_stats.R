...
#Get the output
v<-read.table(var_in,sep="\t",head=T,stringsAsFactors=F,check.names=FALSE)
...
for ( sample in samples_l ){
	...
	X_sum <- sum(v_X[[paste(sample,tot_reads_fld,sep=vg_separator)]])
	#Total reads on Y chromosome 
	Y_sum <- sum(v_Y[[paste(sample,tot_reads_fld,sep=vg_separator)]])
