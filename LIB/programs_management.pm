...
BEGIN
{
	...
	@EXPORT_OK = qw( initialize_folders checkConfigVariables checkVariable
									 ...
									 get_jobid_form_jobname alter_job get_info_from_jobname
...
#BEGIN { print (("  " x $main::x) . "Just used dbmanageent in progmanagement compile\n") }
use LIB::files_manipulation qw( list_to_array delete_file is_folder_empty  
																 );
...
sub get_ped_file_for_enlarged_analysis{
	...
	#Prepare the groupid as field of the query	
...
sub execute_jobs{
  ...
  print_and_log("Executing $jobs_num jobs \n",$log_file);# I'll write a log in $log.\n"; 
	...
	my $analysis_name = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    $cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_name'},
						    $cfg_hash->{'db_analysis_id'},$analsys_id);
	my $job_ids = "";
	...
	foreach my $cmd (@$commands){
			...
			my $job_name = $task."_a".$analsys_id."_$num_cmd";
			print_and_log("\nLaunching a job $job_name : ",$log_file);
			my $job_log = $cfg_hash->{$analsys_id.'_'.$task.'_log_fold'}."/".$analysis_name."_$num_cmd".$cfg_hash->{$task.'_log'};
			#Call the program using qsub and giving it parameters for the job and
			my $env_vars = "COMMAND='$cmd' ";
...
=head2 get_info_from_jobname

 Title   : get_info_from_jobname
 Usage   : get_info_from_jobname( -database => 'name of the database,
                               );
...
=head2 get_jobid_form_jobname

 Title : get_jobid_form_jobname
 Usage : get_jobid_form_jobname( );
...
sub check_config_parameters {
	...
	checkVariable($cfg_hash->{'flagstat_rmdup'},'flagstat_sorted',"",$log_file);
