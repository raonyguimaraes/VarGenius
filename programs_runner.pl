...
use LIB::files_manipulation qw(  tsv_2_xls copy_2_location list_to_array);
...
use LIB::output_management qw( import_vcf_2_db import_ann_out_2_db get_non_covered_regions
																...
																print_alignment_statistics_table);
...
sub run_freebayes_pipeline{
	...
	my $outFile	= "$outFolder/$analysis_name\_freebayes.vcf";
	run_FREEBAYES_vc($cfg_hash,$bam_inputs,$outFile,$analysis_id,$log_file);	
	...
	print_and_log("$analysis_indication [FREEBAYES-pipeline] Executing  $prog_used for filtering VCF file $outFile\n",$log_file);

	my $vcf_output = "$outFolder/$analysis_name\_freebayes.vcf.gz";
	run_GATK_VariantFiltration($cfg_hash,$prog_used,$outFile,$vcf_output,$param_str,$filters,$log_file);
}
...
sub run_alignment_steps{
	...
	if ( $cfg_hash->{'align_mode'} eq 'gatk_BP'){
		...
		if ($cfg_hash->{'sam_sort_idx'} eq 'YES' ){
			...
			#PICARD Sort execution
			my $prog_used = $cfg_hash->{'picard_index_prog'};
			#print_and_log("Starting PICARD $prog_used\n",$log_file);#DEBUGCODE
...
sub run_genotyping_steps{
	...
	#if ( $cfg_hash->{'consensus_vc_approach'} eq 'YES' ){
		#print_and_log("$analysis_indication Starting Consensus Variant Calling approach: the BAM file aligned".
		#"against the genome will be used for variant calling with FreeBayes and Samtools-Mpileup. ".
		#"By default VarGenius adds common variants from these two methods to the result of GATK\n ",$log_file);	
		
		##Path to the programs runner
		#my $program_call = $cfg_hash->{'program_folder'}."/".$cfg_hash->{'programs_runner_script'};
		
		##DEPENDENCIES
		#my $dependency = "no_depend";
		##Freebayes and Samtools are executed using the BAM file merged for lanes, hence it has to wait the
		##stats task which executes the merge
		#my @samplesids = get_samples_id_from_analysis($cfg_hash,$analysis_id,$log_file);
		#foreach my $sampleid (@samplesids){
			#my $jobid = get_jobid_form_jobname($cfg_hash->{'stats_task'}."_s".$sampleid,$cfg_hash->{'qsub_username'});
			#if ( $jobid > 0){
				#$dependency .= "$jobid:";			
			#}else{
				#print_and_log("$analysis_indication ".$cfg_hash->{'stats_task'}." task for sample $sampleid is not in execution. No need to change dependencies... :\n ",$log_file);			
			#}
		#}
		#if ( $dependency ne 'no_depend'){
			#chop($dependency);	
		#}
		#print_and_log("$analysis_indication Freebayes dependencies: $dependency\n ",$log_file);	

		##Run a job for Freebayes pipeline
		##Call the program using qsub and giving it parameters for the job
		#my $task_to_run = $cfg_hash->{'VCFB_task'};	
		#my $job_name = $task_to_run."_a".$analysis_id;
		#my $job_log = $cfg_hash->{$analysis_id.'_'.$task.'_log_fold'}."/".$analysis_name."_freebayes_pipe.log";
		#my $env_vars = "CONFIG_FILE=$config_file,TASK=$task_to_run,SAMPLE_ID=$sample_id,ANALYSIS_ID=$analysis_id,LOG_FILE=$log_file,PIPE=null ";
		#my $fb_job_id = try_exec_job( $cfg_hash,$env_vars,$task_to_run,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file);

		##Run a job for Samtools-mpileup pipeline
		##Call the program using qsub and giving it parameters for the job
		#$task_to_run = $cfg_hash->{'VCSAM_task'};	
		#$job_name = $task_to_run."_a".$analysis_id;
		#$job_log = $cfg_hash->{$analysis_id.'_'.$task.'_log_fold'}."/".$analysis_name."_samtools-mpileup_pipe.log";
		#$env_vars = "CONFIG_FILE=$config_file,TASK=$task_to_run,SAMPLE_ID=$sample_id,ANALYSIS_ID=$analysis_id,LOG_FILE=$log_file,PIPE=null ";
		#my $samvc_job_id = try_exec_job( $cfg_hash,$env_vars,$task_to_run,$program_call,$job_name,$dependency,$job_log,$job_log,$log_file);


		##Now alter the dependencies of the variant filtering job: it has to wait the combine variant job
		#print_and_log("$analysis_indication Getting jobid for".$cfg_hash->{'fastfout_task'}."_a".$analysis_id."\n ",$log_file);
		#my $finalout_jobid = get_jobid_form_jobname($cfg_hash->{'fastfout_task'}."_a".$analysis_id,$cfg_hash->{'qsub_username'});
		#if ( $finalout_jobid > 0){
			#my $new_depend = "-W depend=afterok:$fb_job_id:$samvc_job_id ";
			#print_and_log("$analysis_indication Altering dependencies of ".$cfg_hash->{'fastfout_task'}."_a".$analysis_id." ($finalout_jobid):\n ",$log_file);
			#alter_job($finalout_jobid,$new_depend,$log_file);					
		#}else{
			#print_and_log("$analysis_indication ".$cfg_hash->{'fastfout_task'}." task is not in execution. No need to change dependencies... :\n ",$log_file);			
		#}	
	#}
	...
	if ($cfg_hash->{'genot'} eq 'YES' ){
		...
		if ( $num_samples >= $cfg_hash->{'min_samples_4_joint'} ){		
			...
			my $combvar_job = execute_jobs($cfg_hash,$analysis_id,\@commands,$exec_job_script,$jobs_to_wait,$task,$log_file);

			#Now alter the dependencies of the variant filtering job: it has to wait the combine variant job
			my $new_depend = "-W depend=afterok:$combvar_job ";
			my $varfilt_jobid = get_jobid_form_jobname("varfilt_a".$analysis_id,$cfg_hash->{'qsub_username'});
			...
			if ( -e $mendel_viols_out){
				...
				if ( !-z $mendel_viols_out){
					...
					if (defined $num_mend_viol){
						chomp($num_mend_viol);
						my $line = "Mendelian Violations (lines with at least one undefined genotype (./.) are removed) : $num_mend_viol";
						print_and_log( "$analysis_indication Appending number of mendelian violation to ".$cfg_hash->{$analysis_id."_outstats_file"}." file... \n",$log_file);
						append_str_2_file($cfg_hash->{$analysis_id."_outstats_file"},$line);
...
sub run_reannotate_variants{
	...
	if ( -e $ann_out) {
		...
		#The step to be updated in the db.. HERE I NEED TO UPDATE A TABLE WHERE THERE ARE INFORMATION ABOUT THE DB
...
sub run_coverage_steps{
	...
	if ($cfg_hash->{'flagstat_rmdup'} eq 'YES' ){
		...
		if ( $mergebam_executed ){
			...
			print_and_log( "$analysis_indication I will not start flagstat_rmdup. Duplicates are not removed...\n",$log_file);
		}
	}
...
sub run_coverage_stepsOLD{
	...
	if ( $multiple == 1){
		...
		if ( $mergebam_executed == 1) {
			#Here the files are constructed using the stats_steps_array	
			$bam_input = $inFolder."/".get_output_name_from_executed($params,$cfg_hash->{'sort_idx_step'},
									$params->{$cfg_hash->{'db_sample_name'}},\@stats_steps_array).".".$cfg_hash->{'bam_ext'};
		}
		if ( $mergealn_executed == 1) {
			$bam_aligned = $inFolder."/".get_output_name_from_executed($params,$cfg_hash->{'sort_idx_step'},
									$params->{$cfg_hash->{'db_sample_name'}},\@stats_steps_array)."_".$cfg_hash->{'align_step'}.".".$cfg_hash->{'bam_ext'};				
		}
		print_and_log("\n$analysis_indication Input BAM: $bam_input - BAM aligned only: $bam_aligned..\n",$log_file);#DEBUGCODE
			...
			#If the merged BAM file has not been already produced				
			 if ( !-e $bam_input ){
					if ( $cfg_hash->{'mergebam'} eq 'YES' ){
						...
						#Set the name of the baminput once that it has been created
						$bam_input = $bam_input_merge_sort;
...
#sub run_coverage_stepsOLD{
	...
	#my $bam_input = "";
	##Step is used to build the name
	#my $step = $cfg_hash->{'mergebam_step'};
	##Get the name of the file with the merge of all bam files					
	#my $bed_merged = $inFolder."/".$sample_name."_".$step.".".$cfg_hash->{'bam_ext'};
	
	########CHECK MARK DUPLICATES
	##Check if markDuplicate after the alignment was executed in a previous run
	#my $mrdup_executed = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    #$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'mark_rem_dup_step'},
						    #$cfg_hash->{'db_sample_id'},$sample_id);
						    
	##If the duplicate removal must be executed, get the name of the file with removed duplicates
	#if ( $cfg_hash->{'mark_rem_dup_exec'} eq 'YES'  and $mrdup_executed == 0 ){
		##Checks if mark duplicates has been performed for each read file and update sample status
		#update_sample_status($cfg_hash,$sample_id,$cfg_hash->{'mark_rem_dup_step'});
		##Get mrdup_executed again. So that if it has been just updated we know it
		#$mrdup_executed = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    #$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'mark_rem_dup_step'},
						    #$cfg_hash->{'db_sample_id'},$sample_id);
	#}
	############
	...
	##If there are more read_files per sample than the merge is needed merge them						        						    
	#if ( $multiple == 1){
		#print_and_log( "\n$analysis_indication is sub-divided in multiple samples. A merge is needed..\n",$log_file);
		
		##################
		##Constructs two file names to be used in the merge->remove duplicates->index
		#$step_needed = $cfg_hash->{'mark_rem_dup_step'};
		#my $sample_name = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
							#$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'db_sample_name'},
							#$cfg_hash->{'db_sample_id'},$sample_id);
		...
		#my $bed_merged_rmdup = $inFolder."/".extract_name($bed_merged,1)."_".$cfg_hash->{'mark_rem_dup_step'}.".".$cfg_hash->{'bam_ext'};
		##If the mark and remove duplicate steps has been applied...
		#if ( $mrdup_groups_step_executed or  ($cfg_hash->{'mrdup_groups'} eq 'YES') ){
			##Assign a name to the coverage bed input
			#$bam_input = $bed_merged_rmdup;
		#}else{
			##Assign a name to the coverage bed input
			#$bam_input = $bed_merged;			
		#}
		####################
		...
		##BEDTOOLS_coverageBed execution
			...
			##Merge all the bam files from the alignment in the 
			#my $prog_used = $cfg_hash->{'mergebam_prog'};
			#print_and_log( "RUNNING $prog_used : ",$log_file);#DEBUGCODE
			#run_PICARD_MergeSamFiles_gen($cfg_hash,$prog_used,$samples_to_merge,$bed_merged,$log_file);	
			##Remove duplicates
			##If the mark and remove duplicate step has been applied or will be (after BaseRecalibration)...
			#if ( $mrdup_groups_step_executed or  ($cfg_hash->{'mrdup_groups'} eq 'YES') ){
				#$prog_used = $cfg_hash->{'mrdupbam_prog'};
				#print_and_log( "$analysis_indication  RUNNING $prog_used : ",$log_file);
				#run_PICARD_MarkDuplicates_gen($cfg_hash,$prog_used,$bed_merged,$bed_merged_rmdup,$log_file);
			#}
			##Now sort and index the bam file (it is $bam_input )
			#if (-e $bam_input){
				#my $bam_sort_out = $inFolder."/".extract_name($bam_input,1)."_".$cfg_hash->{'sort_idx_step'}.".".$cfg_hash->{'bam_ext'};
				#print_and_log("$analysis_indication RUNNING  Sort : ",$log_file);
				#run_SAMTOOLS_Sort_gen($cfg_hash,$bam_input,$bam_sort_out,$log_file);
				#print_and_log("$analysis_indication RUNNING  Index : ",$log_file);
				#run_SAMTOOLS_Index_gen($cfg_hash,$bam_sort_out,$log_file);
				#$bam_input = $bam_sort_out;
			#}else{
				#log_and_exit("$analysis_indication File $bam_input does not exists. Please run the alignment...\n",$log_file);
			#}
		...
		#}
		##Here I check if the bam_input has been created successfully, otherwise it must be run
		#if ( -e $bam_input) {
			#print_and_log( "\n$analysis_indication File with merged samples is $bam_input...\n",$log_file);
			
		#}else{
			#log_and_exit( "\n$analysis_indication Cannot find $bam_input. Please run mergebam step..\n",$log_file);
		#}
		...
		#}
	#}else{
		##The last step that could have been executed is mark and remove duplicates
		#$step_needed = $cfg_hash->{'mark_rem_dup_step'};
		...
		##Get the name of the last bam obtained from the alignment task
		#$bam_input = $inFolder."/".get_output_name_from_executed($params,$step_needed,
							#$params->{$main_name},\@steps_array).".".$cfg_hash->{'bam_ext'};
	...
	##############################
		...
		#print_and_log("\n$analysis_indication SAMTOOLS_flagstat execution using the aligned file..\n",$log_file);	
		##If there are more read_files per sample than merge all the sam files						        						    
		#if ( $multiple == 1){
			#print_and_log("\n$analysis_indication Merging all the SAM files in a unique BAM..\n",$log_file);
			#my $main_name = $cfg_hash->{'db_readf_name'};
			##Get the output name
			#$step_needed = $cfg_hash->{'sort_idx_step'};
			#my $step =  $cfg_hash->{'align_step'};
			##Here I give a simple name avoiding the usual usage of the name construction since this is a downstream analysis
			#$aln_input = $inFolder."/".$sample_name."_".$step.".".$cfg_hash->{'bam_ext'};
			#if ( ! -e $aln_input){
				##Here get the read files names to be merged using the sample name and the step needed (align)
				##NB: HO CAMBIATO STEP_NEEDED CON STEP PERCHÈ IN REALTA' QUI MI SERVE IL FILE SAM SOLO ALLINEATO, SENZA SORTING
				#my @paths = get_readf_paths_using_step($cfg_hash,$sample_id,$log_file,$step,$inFolder,\@steps_array,$cfg_hash->{'sam_ext'});

				##my @paths = get_readf_paths_using_step($cfg_hash,$sample_id,$log_file,$step_needed,$inFolder,\@steps_array,$cfg_hash->{'sam_ext'});
				...
				#print_and_log("$analysis_indication RUNNING $prog_used : ",$log_file);
				##Merge all the bam files from the alignment in a single bam 
				#run_PICARD_MergeSamFiles_gen($cfg_hash,$prog_used,$samples_to_merge,$aln_input,$log_file);####COMMENTTOREMOVE###		
				##Remove the SAM files of the aligned and sorted 
				#if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
					#foreach my $path (@paths){
						#print_and_log("$analysis_indication Removing the alignment $path file..\n",$log_file);	
				...
				#}
			#}					
		#}#ELSE the read file corresponds to a single sample
		#else{
			#print_and_log("\n$analysis_indication There are no multiple files..\n",$log_file);
			#my $params;
			#my $main_name = $cfg_hash->{'db_readf_name'};
			#getSampleConfiguration_locked(\$params,$cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
				#$cfg_hash->{'db_pass'},$cfg_hash->{'db_readf_table'},$cfg_hash->{'db_sample_id'},$sample_id);
			##Otherwise take the single aligned bam file 
			#$aln_input = $inFolder."/".build_input_name_from_executed($params,$step_needed,
			#$params->{$main_name},\@steps_array)."_".$step_needed.".".$cfg_hash->{'bam_ext'};
			##If we have to execute flagstat on the bam aligned and sorted we need to 
			##1 get the bam aligned. If only the SAM file is there then convert with view
			#if (! (-e $aln_input) ){
				##Construct the same name with SAM extension
				#$aln_input = $inFolder."/".build_input_name_from_executed($params,$step_needed,
						#$params->{$main_name},\@steps_array)."_".$step_needed.".".$cfg_hash->{'sam_ext'};
				#if (-e $aln_input){
					#print_and_log("$analysis_indication RUNNING  View : ",$log_file);
					##If only the SAM file exists then convert in BAM, sort and index it
					#my $bam_out = $inFolder."/".extract_name($aln_input,1).".".$cfg_hash->{'bam_ext'};
					#run_SAMTOOLS_View_gen($cfg_hash,$aln_input,$bam_out,$log_file);####COMMENTTOREMOVE###
					#my $bam_sort_out = $inFolder."/".extract_name($aln_input,1)."_".$cfg_hash->{'sort_idx_step'}.".".$cfg_hash->{'bam_ext'};
					#print_and_log("$analysis_indication RUNNING  Sort : ",$log_file);
					#run_SAMTOOLS_Sort_gen($cfg_hash,$bam_out,$bam_sort_out,$log_file);####COMMENTTOREMOVE###
					#print_and_log("$analysis_indication RUNNING  Index : ",$log_file);
					#run_SAMTOOLS_Index_gen($cfg_hash,$bam_sort_out,$log_file);####COMMENTTOREMOVE###
					#$aln_input = $bam_sort_out;
					##Remove the BAM files not sorted
					#if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
						#delete_file($bam_out);						
					#}						
				#}else{
						#log_and_exit("$analysis_indication File $aln_input does not exists. Please run the alignment...\n",$log_file);
				#}
			#}
		#}
		...
		#my $prog_used = $cfg_hash->{'flagstat_prog'};	
		#print_and_log("$analysis_indication RUNNING $prog_used : \n",$log_file);		
		#$flagstat_aln = $outFolder."/".extract_name($aln_input,1).".".$cfg_hash->{'flagstat_ext'};	
		#run_SAMTOOLS_flagstat($cfg_hash,$aln_input,$flagstat_aln,$log_file);####COMMENTTOREMOVE###
		...
		##Remove the SAM files of the aligned and sorted used for the statistics
		#if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
			#print_and_log("$analysis_indication Removing the alignment $aln_input file..\n",$log_file);	
			#delete_file($aln_input);						
		#}	
...
sub run_fast_final_out_steps{
	...
	##Depending by the sequencing type we will decide to annotate a different
	#my $previous_task;
	##VCF file. 
	## If it is an exome with a Trio, the phasing has been performed
	#my $sequencingtype = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
									#$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_sequencingtype'},
									#$cfg_hash->{'db_analysis_id'},$analysis_id);		
	
	##Getting the pedigree file to check if this is a trio
	#my $ped_file = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    #$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_pedfile'},
						    #$cfg_hash->{'db_analysis_id'},$analysis_id);	
	#my $ped_f_path = $cfg_hash->{$analysis_id.'_data_fold'}."/$ped_file";
	
	##If the analysis is of a Trio then the phasing has been executed
	##(When using VQSR the PED file is created but the db field is not filled)
	#if ( -e $ped_f_path ){
		#$previous_task	= 'phasing';
		
	#}else{
		#$previous_task	= 'varfilt';	
	#}
	...
	if ( $consensus ){
		...
		#Then I have to get the variants from the common that are not in the results from the GATK
		print_and_log("$analysis_indication Get the variants from $vcf_intersect that are not in GATK $vcf_file..\n",$log_file);	
		my $vcf_difference = "$inFolder/$analysis_name\_gatk-fb_intersect.vcf";
		run_BEDTOOLS_subtractBed($cfg_hash,$vcf_intersect,$vcf_file,$vcf_difference,$params,$log_file);
		...
		my $vcfs = "$vcf_file,$vcf_difference";
		my $final_vcf = "$inFolder/$analysis_name\_gatk_fb.vcf";
		#run_GATK_CombineVariants($cfg_hash,$cfg_hash->{'combvar_prog'},$vcfs,$final_vcf,$log_file,"NO");#CON SAMTOOLS
		
		#Join the two VCF files
		print_and_log("$analysis_indication Joining $vcf_file and $vcf_difference into $final_vcf.temp..\n",$log_file);			
		join_files_with_cat($vcf_file,$vcf_difference,$final_vcf.".temp");
		...
		run_BEDTOOLS_subtractBed($cfg_hash,$vcf_file,$vcf_intersect,$vcf_gatk_uniq,$params,$log_file);

		#Delete all temporary files
		if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
			print_and_log("$analysis_indication Removing the following temporary files: $vcf_difference\n$final_vcf.temp..\n",$log_file);	
			delete_file($vcf_difference);
  ...
  if ($genotref == 1){
	...
	if ($cfg_hash->{'rearrange_output'} eq 'YES'){
		...
		if ( -e $output_2_rearrange and !(-z $output_2_rearrange)) {
			if ( correct_type($resgroupid,"positiveint")){							
				...
				rearrange_rawtabular_out($cfg_hash,$output_2_rearrange,$rearr_ann_out);
	...
	if ($cfg_hash->{'update_overall_samples_plots'} eq 'YES'){
		...
		print_and_log("$analysis_indication Getting all samples reads number plot... \n",$log_file);
...
sub outlist_to_hash{
 ...
 while (my $line = <FILE>){
	...
	#$$paths_hash->{$fields[$page_fld_ind]}->{$fields[$samid_fld_ind]}->{$fields[$rdf_fld_ind]}->{'desc'} = $fields[$desc_fld_ind];
	#$$paths_hash->{$fields[$page_fld_ind]}->{$fields[$samid_fld_ind]}->{$fields[$rdf_fld_ind]}->{'path'} = $fields[$path_fld_ind];
